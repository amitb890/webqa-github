...
function displayAlertSwithProject(classVal, data){
            ...
            if(data.xmldata != ""){
              if(data.xmldata.length > 0){
                ...
                data.xmldata.forEach((url, i)=>{
                  list+= i === data.xmldata.length-1 ? url : url + "\n"
                })
                $("#urlsList").html(list)
  ...
  function getFormattedName(url) {
    ...
    // Split the domain by dots and get the second-level domain
    let parts = domain.split('.');
    let name = parts.length > 2 ? parts[parts.length - 2] : parts[0];
    // Capitalize the first letter and return
    $('.onbording_project_name').val(name.charAt(0).toUpperCase() + name.slice(1))
...
// Feature Child Settings, Security settings section 
// function featureChildPageSecSet(){
//   const cardsWrapper = document.querySelector(".fcs6-cards-wrapper");
...
// if (cardsWrapper && cardsContainer && leftBtn && rightBtn) {
//   let currentIndex = 0;
// const cardWidth = 250; // card width + gap
// const totalCards = document.querySelectorAll(".fcs6-card").length;
// const visibleCards = Math.floor(cardsWrapper.offsetWidth / cardWidth); 
// const maxIndex = totalCards - visibleCards;

// function updateButtons() {
//   leftBtn.style.backgroundColor = currentIndex <= 0 ? "#e4e4e4" : "#2b56a5";
//   rightBtn.style.backgroundColor = currentIndex >= maxIndex ? "#e4e4e4" : "#2b56a5";
// }
