...
function wthf_after_post_content($content){
	...
	if(is_singular($selected_post_types)){
		...
		if(!isset($_COOKIE["helpful_id_".$post_id])){
	    	...
	    	$content .= '<div class="getting-recover-main  helpfull-area"><div id="was-this-helpful" data-post-id="'.$post_id.'" data-thank-text="'.get_option("wthf_thank_text").'"><div id="wthf-title">'.get_option("wthf_question_text").'</div><div id="wthf-yes-no"><span data-value="1">'.get_option("wthf_yes_text").'</span><span data-value="0">'.get_option("wthf_no_text").'</span></div></div></div>';
