<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Feature Root - Page</title>

    @include('components.imran-components.meta-tags-css-links')
</head>
<body style="background-color: #fff;">


    <!-- header starts -->
        @include('components.imran-components.header')
        <!-- header ends -->
    

    <!-- Section 1 starts -->
     <section class="fr1">
        <div class="fr1-d1">
            <h1 class="fr1-d1-h">Feature to Make Website Management Easy</h1>
            <p class="fr1-d1-p">
                We analyze every element — design, performance, SEO, and user experience to uncover growth opportunities.
            </p>
            <div>
                <button class="fr1-d1-button">
                    Sign up Free
                </button>
            </div>
        </div>
        <div class="fr1-d2">
            <img class="fr1-d2-img" src="{{ asset('raw-files-css-js/assets/images/feature-root-page/Group 1064.svg') }}" alt="">
        </div>
     </section>
    <!-- Section 1 ends -->


    <!-- Section 2 Starts -->
     <section class="fr2">
        <div class="fr2-d">
            <h1 class="fr2-d-h">
                Website Auditing
            </h1>
            <p class="fr2-d-p">
                Our Customized Auditing Settings let you define exactly what matters for your website’s success. Choose specific metrics, focus areas, and performance goals tailored to your business. From SEO to speed optimization.
            </p>
            <img class="fr2-d-img" src="{{ asset('raw-files-css-js/assets/images/feature-root-page/Group 1017.svg') }}" alt="">
            <div class="fr2-d-d">
                <h6 class="fr2-d-d-h6">Features that make auditing effortless</h6>
                <div class="fr2-d-d-d container">
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            SEO Performance
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            User Experience
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            Website Security
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                </div>
                <div>
                    <button class="fr1-d1-button">
                        Sign up Free
                    </button>
                </div>
            </div>
        </div>
     </section>
    <!-- Section 2 Ends -->


    <!-- Section 3 Starts -->
     <section class="fr3">
        <div class="fr3-d">
            <div class="fr3-d-h">
                <h1 >
                    Customized Auditing Settings
                </h1>
            </div>
            <p class="fr3-d-p">
                Explore organic and paid traffic metrics for any website using Site Explorer. Use the interactive graph to see how traffic has progressed both globally and locally in 171 countries.
            </p>
            <img class="fr3-d-img" src="{{ asset('raw-files-css-js/assets/images/feature-root-page/Group 1036 (1).svg') }}" alt="">
            <div class="fr3-d-d">
                <h6 class="fr3-d-d-h6">Features that make auditing effortless</h6>
                <div class="fr3-d-d-d container">
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            SEO Performance
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            User Experience
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            Website Security
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                </div>
                <div>
                    <button class="fr1-d1-button">
                        Sign up Free
                    </button>
                </div>
            </div>
        </div>
     </section>
    <!-- Section 3 Ends -->

    <!-- Section 4 starts -->
     <section class="fr4">
        <div class="fr4-inner">
            <div class="fr4-d1">
                <h3>Increase Your Website Performance with Premium Features</h3>
                <p>Explore organic and paid traffic metrics for any website using Site Explorer.</p>
                <div>
                    <button class="fr4-d1-button">
                        Sign Up Free
                    </button>
                </div>
            </div>
            <div class="fr4-d2" id="fr4-d2">
                <img src="{{ asset('raw-files-css-js/assets/images/feature-root-page/image-71.svg') }}" alt="">
            </div>
        </div>
     </section>
    <!-- Section 4 ends -->


    <!-- Section 5 Starts -->
     <section class="fr2">
        <div class="fr2-d">
            <h1 class="fr2-d-h">
                Website Tracker
            </h1>
            <p class="fr2-d-p">
                Our Customized Auditing Settings let you define exactly what matters for your website’s success. Choose specific metrics, focus areas, and performance goals tailored to your business. From SEO to speed optimization.
            </p>
            <img class="fr2-d-img" src="{{ asset('raw-files-css-js/assets/images/feature-root-page/Group 1036 (2).svg') }}" alt="">
            <div class="fr2-d-d">
                <h6 class="fr2-d-d-h6">Features that make auditing effortless</h6>
                <div class="fr2-d-d-d container">
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            SEO Performance
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            User Experience
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr2-d-d-dDivs">
                        <h6>
                            Website Security
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                </div>
                <div>
                    <button class="fr1-d1-button">
                        Sign up Free
                    </button>
                </div>
            </div>
        </div>
     </section>
    <!-- Section 5 Ends -->

    <!-- Section 6 Starts -->
     <section class="fr3">
        <div class="fr3-d">
            <div class="fr3-d-h">
                <h1 >
                    Website Reports
                </h1>
            </div>
            <p class="fr3-d-p">
                Explore organic and paid traffic metrics for any website using Site Explorer. Use the interactive graph to see how traffic has progressed both globally and locally in 171 countries.
            </p>
            <img class="fr3-d-img" src="{{ asset('raw-files-css-js/assets/images/feature-root-page/Group 1037.svg') }}" alt="">
            <div class="fr3-d-d">
                <h6 class="fr3-d-d-h6">Features that make auditing effortless</h6>
                <div class="fr3-d-d-d container">
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            SEO Performance
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            User Experience
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                    <div class="fr3-d-d-dDivs">
                        <h6>
                            Website Security
                        </h6>
                        <p>Identifies issues like broken links, slow page speed, and poor keyword.</p>
                    </div>
                </div>
                <div>
                    <button class="fr1-d1-button">
                        Sign up Free
                    </button>
                </div>
            </div>
        </div>
     </section>
    <!-- Section 6 Ends -->

    <!-- Section last starts -->
     <section class="frlast">
        <div class="frlast-inner">
            <div class="frlast-d1">
                <h3>Increase Your Website Performance with Premium Features</h3>
                <p>Explore organic and paid traffic metrics for any website using Site Explorer.</p>
                <div>
                    <button class="fr4-d1-button">
                        Sign Up Free
                    </button>
                </div>
            </div>
            <div class="frlast-d2">
                <img src="{{ asset('raw-files-css-js/assets/images/feature-root-page/young-person-intership-Photoroom 1.svg') }}" alt="">
            </div>
        </div>
     </section>
    <!-- Section last ends -->

    <!-- Footer Area Start -->
    @include('components.imran-components.footer')
    <!-- Footer Area End -->


    
    @include('components.imran-components.js-scripts')
</body>
</html>