...
<img src="{{ asset('new-assets/assets/images/bulk-tool/unsafe-cross-origin-links-cover.png') }}" alt="Unsafe cross origin links definition" width="600" height="400" class="img-fluid my-4">
...
<h3>How Does Reverse Tabnabbing Work?</h3>

<p>
  Reverse tabnabbing takes advantage of the relationship between an original
  page and a page opened in a new browsing context. If the destination page
  receives a reference to the original page through <b>window.opener</b>, it
  may be able to navigate that original page.
</p>

<div class="list yellow-content">
  <p><b>Here is how reverse tabnapping works through a sequence - </b></p>
  <ol>
    <li>
      A visitor is on your website and clicks an external link.
    </li>
    <li>
      The link opens the external destination in a new tab using
      <b>target="_blank"</b>.
    </li>
    <li>
      In older or incorrectly configured browsing environments, the new page
      may have access to <b>window.opener</b>.
    </li>
    <li>
      A compromised or malicious destination could attempt to navigate the
      original tab to a phishing page.
    </li>
    <li>
      Because the visitor originally trusted the first website, the redirected
      page may appear more convincing.
    </li>
  </ol>
</div>


<img src="{{ asset('new-assets/assets/images/bulk-tool/reverse-tabnapping.png') }}" alt="Unsafe cross origin links reverse tabnapping" width="600" height="400" class="img-fluid my-4">
<p>
  The protection provided by <b>rel="noopener"</b> is designed to prevent the
  newly opened page from receiving this opener relationship.
</p>

<h4>Do we "Always" Need to Open External Links in a New Tab?</h4>

<p>
  Not necessarily. Opening a link in a new tab should be a deliberate user
  experience decision rather than a default rule for every external link.
</p>

<p>
  Standard links normally open in the current browsing context, allowing users
  to use the browser's Back button to return to the previous page. A new tab
  can be useful when users need to keep the current page open while consulting
  another resource.
</p>

<ul>
  <li><b>Use a new tab when:</b> the user benefits from keeping the current workflow open.</li>
  <li><b>Use the current tab when:</b> the destination is simply the next step in the user's journey.</li>
  <li><b>Avoid unnecessary new tabs:</b> especially when many links on the same page open separate tabs.</li>
</ul>

<p>
  The goal is to balance security, privacy, accessibility, and user
  experience rather than opening every external link in a new tab.
</p>

<h3>Some Situations when Unsafe Cross Origin Links are Generated</h3>

<p>
  Unsafe link patterns often appear because links are generated by different
  parts of a website rather than through a single reusable component.
</p>

<ul>
  <li>
    <b>Manually added external links</b> — A developer adds
    target="_blank" but forgets the appropriate rel
    attribute. This is the most common reason, and the explanation here is a human oversight or lack of awareness of using the correct rel attributes.
  </li>
  <li>
    <b>CMS generated links</b> — Content editors or plugins automatically add
    new tab behavior without applying the site's preferred security pattern.
  </li>
  <li>
    <b>Third party website widgets</b> — Embedded components generate external links
    independently of the site's normal templates.
  </li>
  <li>
    <b>User generated content</b> — Comments, profiles, forums, or other
    areas allow users to submit external URLs. These systems may not always add the appropriate rel attributes to prevent unsafe cross origin external links.
  </li>
  <li>
    <b>Reusable UI components</b> — Buttons, cards, navigation components, or
    promotional blocks may use different link patterns.
  </li>
  <li>
    <b>JavaScript created windows</b> — Links opened through
    <code>window.open()</code> require their own opener handling. If proper rel attributes are not added to Javascript code, sometimes it may result in the creation of unsafe cross origin links.
  </li>
</ul>
